...
public class coralbow extends BowItem {
   public coralbow(Item.Settings settings) {
      super(settings);
         ...
         if (!itemStack.isEmpty() || bl) {
            ...
            if ((double)f >= 0.1D) {
               ...
               if (!world.isClient) {
                  ...
                  if (j > 0) {
                     projectileEntity.setDamage(projectileEntity.getDamage() + (double)j * 0.7D + 0.5D);
                  }
                  ...
                  if (EnchantmentHelper.getLevel(Enchantments.FLAME, stack) > 0) {
                     projectileEntity.setOnFireFor(120);
                  }
